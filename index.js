...
function addHistoryEntry(url, count, status) {
  const history = loadHistory();
  const time = new Date().toISOString();
  history.unshift({ time, url, count, status });
  saveHistory(history);
}
...
app.post('/api/submit', async (req, res) => {
  ...
  try {
    ...
    if (!cookies) return res.status(400).json({ status: 500, error: 'Invalid cookies' });

    await share(cookies, url, amount, interval);
    addHistoryEntry(url, amount, 'Success');
    res.status(200).json({ status: 200 });

  } catch (err) {
    addHistoryEntry(url, amount, 'Failed');
...
async function share(cookies, url, amount, interval) {
  ...
  if (!id) throw new Error("Unable to get link id: invalid URL, private or friends-only post");
